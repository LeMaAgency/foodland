...
class Section
{
    ...
    {
        if(empty($this->arItem['PICTURE']))
            return false;
        return empty($this->arItem['PICTURE'][$key]) ? null : $this->arItem['PICTURE'][$key];
    }
    ...
    public function detailPicture($key = 'SRC')
    {
        if(empty($this->arItem['DETAIL_PICTURE']))
            return false;
        $picture = \CFile::GetPath($this->arItem['DETAIL_PICTURE']);
        return empty($picture) ? null : $picture;
    }
